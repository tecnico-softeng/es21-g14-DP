...
class ImportExportMultipleChoiceQuestionsTest extends SpockTest {
    ...
    def 'export and import questions to xml multiple answer'() {
        given: 'a xml with questions'
        def questionsXml = questionService.exportQuestionsToXml()
        print questionsXml
        and: 'a clean database'
        ...
        optionFourResult.getContent() == OPTION_1_CONTENT
        !(!(optionOneResult.isCorrect()) && !(optionTwoResult.isCorrect()) && !(optionThreeResult.isCorrect()) && !(optionFourResult.isCorrect()))
    }
